...
double *create_empty_vector(size_t len) {
    double *vector;
    vector = (double *) calloc(len, sizeof(double));
    return vector;
}
...
double **sub_matrix_sequential(double **mat1, double **mat2, double **resultMat, int row1, int col1) {
    for (int i = 0; i < row1; i++)
        for (int j = 0; j < col1; j++)
...
double **mul_matrix_scalar_sequential(double **mat1, double **resultMat, double scalar, size_t row1, size_t col1) {
    for (int i = 0; i < row1; i++)
        for (int j = 0; j < col1; j++)
            resultMat[i][j] = mat1[i][j] * scalar;
    return resultMat;
}
...
double *mul_matrix_vector_sequential(double **Ainv, double *u, double *resultVec, size_t row_mat, size_t col_mat) {
    for (int i = 0; i < row_mat; i++) {
        double dot = 0;
        for (int j = 0; j < col_mat; j++)
            dot += Ainv[i][j] * u[j];
...
double *mul_vector_matrix_sequential(double **Ainv, double *v, double *resultVec, size_t row_mat, size_t col_mat) {
    for (int i = 0; i < col_mat; i++) {
        double dot = 0;
...
double *
mul_vector_matrix_parallel_with_matrix_transpose(double **matrix, double *vector, size_t row_mat, size_t col_mat,
                                                 size_t len) {
    if (len != row_mat) {
        fprintf(stderr, "incompatible matrix and vector sizes for multiplication\n");
        exit(EXIT_FAILURE);
...
double *
mul_vector_matrix_sequential_with_matrix_transpose(double **matrix, double *vector, size_t row_mat, size_t col_mat,
                                                   size_t len) {
    if (len != row_mat) {
        fprintf(stderr, "incompatible matrix and vector sizes for multiplication\n");
        exit(EXIT_FAILURE);
...
double **outer_vector_vector_sequential(double *vec1, double *vec2, double **resultMat, size_t len1, size_t len2) {
    for (int i = 0; i < len1; i++)
        for (int j = 0; j < len2; j++)
            resultMat[i][j] = vec1[i] * vec2[j];
    return resultMat;
}
...
double **
sherman_morrison_sequential(double **A_inv, double *u, double *v, double **resultMat, double *resultVec, size_t size) {

    //num1 = (A^-1)(u) --size--> n*1
    //num2 = (v^T)(A^-1) --size--> 1*n
    //numerator --size--> n*n
    double *num1, *num2, **numerator, **second_term;


    outer_vector_vector_sequential(num1, num2, numerator, size, size);

    //den1 = (v^T)(A^-1) = num2 --size--> 1*n
    double denominator = 1 + inner_vector_vector_sequential(num2, u, size);

    mul_matrix_scalar_sequential(numerator, second_term, 1.0 / denominator, size, size);
    sub_matrix_sequential(A_inv, second_term, resultMat, size, size);
    return resultMat;
